...
function filterDates(dates: MatchDateGroup[], league: string, status: string) {
  return dates
    .map((group) => ({
      ...group,
...
function ScheduleRow({ match, revealSpoiler }: { match: MatchListItem; revealSpoiler: boolean }) {
  ...
  return (
    ...
    >
      <div className="flex flex-row items-baseline justify-center gap-2 pt-1 sm:flex-col sm:items-start sm:justify-start">
          ...
          <div className="flex flex-col items-center gap-1.5">
            <span
              className={cn(
                ...
                hideScore ? "select-none blur-[5px] opacity-75" : "",
              )}
            >
              {mobileCenterLabel}
            </span>
          ...
          <div className="grid min-w-0 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-5">
            ...
            <span
              className={cn(
                ...
                hideScore ? "select-none blur-[6px] opacity-75" : "",
              )}
            >
              {leftScore}
            </span>
...
export function HupuScheduleExplorer({
  months,
  selectedMonthId,
  mode,
}: {
  ...
  const initialMonthId = getDefaultMonthId(months, selectedMonthId ?? undefined);
  const initialWeekId = months.find((month) => month.id === initialMonthId)?.weeks[0]?.id ?? "";

  const [league, setLeague] = useState("all");
  const [status, setStatus] = useState<"all" | "scheduled" | "finished">("all");
  const [currentMonthId, setCurrentMonthId] = useState(initialMonthId);
  ...
  const leagues = useMemo(
    () => ["all", ...new Set(months.flatMap((month) => month.weeks.flatMap((week) => week.dates.flatMap((date) => date.matches.map((match) => match.league)))))],
    [months],
  );

  const selectedMonth = months.find((month) => month.id === currentMonthId) ?? months[0] ?? null;
  const selectedWeek = selectedMonth?.weeks.find((week) => week.id === currentWeekId) ?? selectedMonth?.weeks[0] ?? null;
  const visibleWeekDates = useMemo(() => (selectedWeek ? filterDates(selectedWeek.dates, league, status) : []), [selectedWeek, league, status]);
    ...
    <section id="schedule-explorer" className="mx-auto w-full max-w-5xl overflow-hidden rounded-[20px] bg-[#31313C] text-white shadow-[0_12px_28px_rgba(2,6,23,0.28)]">
      <div className="bg-[#31313C] px-4 py-3 sm:px-5">
        <ScheduleFilterBar
          ...
          onMonthChange={(value) => {
            setCurrentMonthId(value);
            const month = months.find((item) => item.id === value);
            setCurrentWeekId(month?.weeks[0]?.id ?? "");
          }}
          weekOptions={selectedMonth?.weeks ?? []}
          selectedWeekId={currentWeekId}
          onWeekChange={(value) => setCurrentWeekId(value)}
        />
      </div>
